...
## テスト計画
- [ ] `vuljit/analysis/research_question3/tests/test_additional_build_strategies.py` に WalkForward 用モックデータを追加し、Fold 単位の学習・推論分割と特徴量リスト同期を検証する。
- [ ] 前処理ユーティリティの単体テストを `scripts/modeling` モジュールと共用し、特徴量欠損・列追加時のフォールバック挙動を確認する。
...
## 対象コード/資産
- [ ] `vuljit/analysis/research_question3/additional_build_strategies.py:954-1159` の改修対象を洗い出す。
- [ ] `/work/riku-ka/vuljit/scripts/modeling` パイプラインと Strategy4 の依存関係を整理する。
- [ ] `scripts/modeling/settings.py`, `data_preparation.py`, `model_definition.py`, `evaluation.py`, `main_per_project.py` の共通化ポイントを特定する。
- [ ] `datasets/raw/rq3_dataset/*.csv`, `datasets/derived_artifacts/rq3/timeline_outputs/build_timelines/*.csv`, `scripts/modeling` ディレクトリ内の特徴量結合 CSV の入出力要件を確認する。
...
### 2. 特徴量・前処理の共通化
- [x] `scripts/modeling/settings.py` の `predicted_risk_VCCFinder_Coverage` 特徴量集合（`KAMEI_FEATURES` + `VCCFINDER_FEATURES` + Coverage 系列 + ベース指標）をデフォルト採用する。
...
- [ ] `vuljit/analysis/research_question3/docs/additional_build_strategies_detail.md` に Strategy4 の新プロセス・使用特徴量・評価結果を追記する。
- [ ] `docs/reports/strategy4_rework_results.md` を作成し詳細レポートをまとめる。
- [ ] `scripts/modeling` モジュール README/ドキュメントへ共通化した前処理・モデル仕様を追記する。
